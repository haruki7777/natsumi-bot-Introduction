...
function toggleCommandList(categoryId) {
  ...
  if (commandList && helpContent) {
    const isActive = commandList.classList.toggle('active');
    helpContent.classList.toggle('active', !isActive);
  }
}
...
function showDescription(command) {
  ...
  const koreanCommand = document.getElementById('fullScreenKoreanCommand');
  const descriptionText = document.getElementById('fullScreenText');
  ...
  if (overlay && koreanCommand && descriptionText && commandDescriptions[command]) {
    koreanCommand.textContent = koreanCommandMap[command] || `/${command}`;
    descriptionText.textContent = commandDescriptions[command];
